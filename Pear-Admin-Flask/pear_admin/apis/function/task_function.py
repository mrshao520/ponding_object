...
def task_function_crawling(
    ...
    # 配置文件，保存的文件路径
    untreated_filename = BaseConfig.UNTREATED_DATA_FILE
...
def task_function(
    ...
    # 配置文件，保存的文件路径
    untreated_filename = BaseConfig.UNTREATED_DATA_FILE
    csv_filename = BaseConfig.DATA_FILE
    ...
    if not BaseConfig.OPEN_PONDING_SERVER:
        ...
        logger.info(
            f"爬取有效信息个数/总有效信息个数: {effective_number}/{channel_info.total_effective_number}"
        )
    # 保存 渠道 信息
    with scheduler.app.app_context():
        channel_info.save()
